...
public class HORRIBLE_Lazy {
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		int n, c, p, q, v;
		long[] arr;
		SegmentTree_HORRIBLE_Lazy segTree;
		while (t-- > 0) {
			input = bf.readLine().trim().split(" ");
			n = Integer.parseInt(input[0]);
			c = Integer.parseInt(input[1]);
			arr = new long[n];
			segTree = new SegmentTree_HORRIBLE_Lazy(arr, n);
			while (c-- > 0) {
				input = bf.readLine().trim().split(" ");
				switch (input[0]) {
				case "0":
...
class SegmentTree_HORRIBLE_Lazy {
	int n;
	long[] st;
	long[] arr;
	long[] lazy;

	public SegmentTree_HORRIBLE_Lazy(long[] arr, int n) {
		this.n = n;
	...
	private long queryUtil(int left, int right, int start, int end, int current) {
		...
		if (lazy[current] != 0) {
			st[current] += (long)(end - start + 1) * lazy[current];
			if (start != end) {
	...
	private void addUtil(int left, int right, int start, int end, int current, int val) {
		...
		if (left <= start && right >= end) {
			st[current] += (long)(end - start + 1) * val;
			if (start != end) {
				lazy[2 * current + 1] += val;
				lazy[2 * current + 2] += val;
			}
			return;
		}
		int mid = getMid(start, end);
