...
public class DIVMAC_ED {
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for (int i = 0; i < t; i++) {
			...
			sTree = new SegmentTreeTwo(arr, n, lpd);
...
class SegmentTreeTwo {
	int[] st;
	int[] lpd;

	public SegmentTreeTwo(int[] arr, int n, int[] lpd) {
	...
	public int update(int[] arr, int start, int end, int current, int l, int r) {
		...
		if (start == end) {
			st[current] = lpd[arr[start] / lpd[arr[start]]];
